	...
	*/
	if ((dwRetVal = GetIpForwardTable(pIpForwardTable, &dwSize, 0)) == NO_ERROR) {
		...
		for (i = 0; i < (int)pIpForwardTable -> dwNumEntries; i++) {
			...
			inet_ntop(AF_INET, &(IpAddr.S_un.S_addr), szDestIp, INET_ADDRSTRLEN);
			//strcpy_s(szDestIp, sizeof(szDestIp), inet_ntoa(IpAddr));

			IpAddr.S_un.S_addr = (u_long)pIpForwardTable -> table[i].dwForwardMask;
			inet_ntop(AF_INET, &(IpAddr.S_un.S_addr), szMaskIp, INET_ADDRSTRLEN);
			//strcpy_s(szMaskIp, sizeof(szMaskIp), inet_ntoa(IpAddr));

			IpAddr.S_un.S_addr = (u_long)pIpForwardTable -> table[i].dwForwardNextHop;
			inet_ntop(AF_INET, &(IpAddr.S_un.S_addr), szGatewayIp, INET_ADDRSTRLEN);
